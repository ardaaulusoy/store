<?php
/*
 * Plugin Name: WooCommerce Simple Auction | Shared By Themes24x7.com
	...
	if (!class_exists('WooCommerce_simple_auction')) {
		class WooCommerce_simple_auction {
			...
			public function init() {
				...
				// add_filter('plugins_api_result', array($this,'plugin_api_call'), 10, 3);
			...
			public function woocommerce_simple_auctions_admin_notice() {
				...
				if (current_user_can('manage_options')) {
					...
					if (isset($auction_closing_soon_settings['enabled']) && $auction_closing_soon_settings['enabled'] == 'yes' && get_option('Woocommerce_simple_auction_cron_closing_soon_emails') != "yes" && !get_user_meta($user_id, 'closing_soon_emails_ignore_notice')) {
						echo '<div class="updated">
					   	<p>' . sprintf(__('For ending soon emails please set up cronjob every 30 minutes: <b>%s/?auction-cron=closing-soon-emails</b>. | <a href="%s">Hide Notice</a>', 'wc_simple_auctions'), get_bloginfo('url'), esc_attr(add_query_arg('closing_soon_emails_ignore', '0'))) . '</p>
						</div>';
					}
				}
			}
			...
			public function admin_enqueue_script($hook) {
				...
				if ($hook == 'post-new.php' || $hook == 'post.php' || $hook == 'woocommerce_page_auctions-activity') {
					if ('product' == get_post_type() || $hook == 'woocommerce_page_auctions-activity') {
						...
						wp_localize_script('simple-auction-admin', 'SA_Ajax', array('ajaxurl' => admin_url('admin-ajax.php'), 'SA_nonce' => wp_create_nonce('SAajax-nonce') ,'calendar_image'=> WC()->plugin_url() . '/assets/images/calendar.png'));
			...
			public function frontend_enqueue_script() {
				...
				wp_localize_script('simple-auction-frontend', 'SA_Ajax', array('ajaxurl' => add_query_arg('wsa-ajax', ''), 'najax' => TRUE, 'last_activity' => get_option('simple_auction_last_activity','0'), 'focus' => get_option('simple_auctions_focus','yes')));
...
Use upper-case characters for mandatory periods, or the corresponding lower-case characters for optional periods, i.e. only display if non-zero. Once one optional period is shown, all the ones after that are also shown.", 'wc_simple_auctions'),
			 ...
			 *
			 */
			function auction_payed($order_id) {
			...
			function auction_order_canceled($order_id) {
					$order = wc_get_order($order_id);
			...
			*/
			...
			function remove_auction_from_user_metafield($data){
			...
			function change_last_activity_timestamp($data){
			...
			function check_for_plugin_update($checked_data) {
				...
				$request_string = array(
						'body' => array(
							...
							'purchase-code' => get_option('simple_auctions_purchase_code')
						),
			...
			public function remove_finished_auctions_from_related_products($query){
				...
				if ( count( $finished_auctions ) && $simple_auctions_finished_enabled == 'no' ) {
						$query['where'] .= " AND p.ID NOT IN ( " . implode( ',', array_map( 'absint', $finished_auctions ) ) . " )";
				}
				if ( count( $future_auctions ) && $simple_auctions_future_enabled == 'no' ) {
