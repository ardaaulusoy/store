...
class TAWC_Deals_Admin {
	...
	public function add_deal_fields() {
		...
		<p class="form-field _deal_quantity_field">
			...
			<?php
			if ( $quantity > 0 ) {
				echo '<span class="deal-sold-counts" style="clear:both;display:block;"><strong>' . sprintf( _n( '%s product is sold', '%s products are sold', max( 1, $sales_counts ), 'tawc-deals' ), $sales_counts ) . '</strong></span>';
			}
			?>
		</p>
